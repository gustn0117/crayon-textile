import { telHref, type SiteInfo } from "@/lib/siteInfo";

export function ko(info: SiteInfo) {
  return {
    htmlLang: "ko",
    ...
    /* Display formats — each locale spells the numbers its own way. */
    phone: {
      tel: info.tel,
      mobile: info.mobile,
      fax: info.fax,
    ...
    home: {
      ...
      stories: [
        ...
        {
          index: "03",
          title: ["도안은 사서 쓰지 않고,", "직접 그립니다."],
          body: "자체 디자인 제도실에서 아이디어 구상부터 패턴 개발, 컬러웨이 전개까지 직접 관리합니다. 카피가 아닌 독자 개발 패턴이라 같은 원단이 시장에 흩어져 있을 위험이 적습니다.",
          link: "STUDIO",
          caption: "패턴 개발",
        },
      ],
      ...
      closingHeadline: "찾고 계신 원단을 함께 이야기해 보세요.",
      closingAction: "원단 · 패턴 상담하기",
    ...
    about: {
      ...
      profile: [
        ...
        { term: "소재지", value: info.addressKo.filter(Boolean).join(" ") },
        { term: "취급 품목", value: "나염 프린트 원단, 텍스타일 패턴 디자인" },
        { term: "주요 분야", value: "여성복 · 아동복 · 캐주얼 · 침구 및 홈패브릭" },
        { term: "공급 지역", value: "대한민국, 일본, 대만, 유럽, 중동" },
        { term: "연락처", value: `T. ${info.tel} / M. ${info.mobile}` },
      ],
      profileNote:
    ...
    studio: {
      ...
      roomAlts: [
        "크레용 디자인 제도실에서 디자이너들이 패턴 도안과 컬러 자료를 놓고 작업하는 모습",
        "모니터에 띄운 나염 패턴을 보며 배색을 검토하는 크레용 디자인팀",
        "디자인 제도실 회의 공간",
      ],
    ...
    contact: {
      ...
      visitHead: {
        en: "VISIT & CONTACT",
        note: info.addressKo[1] || info.addressKo[0],
        title: "오시는 길과 연락처.",
      },
      storeAlt:
        "동대문종합시장 D동 2621호 크레용 매장. 파란 간판 아래로 원단 스와치와 샘플, 완성된 의류가 진열되어 있다.",
      storeCaption: "이 간판을 찾으시면 됩니다",
      details: [
        { term: "주소", lines: info.addressKo.filter(Boolean) },
        { term: "전화", lines: [`T. ${info.tel}`, `M. ${info.mobile}`] },
        ...(info.fax ? [{ term: "팩스", lines: [`F. ${info.fax}`] }] : []),
        { term: "이메일", lines: [info.email] },
      ],
      mapLink: "지도에서 보기",
      visitNote:
...
export type Dictionary = ReturnType<typeof ko>;
